...
py_require(c("matplotlib", "polars")) 
...
Zur Bearbeitung von Aufgaben aus dem Bereich der Stochastik und Statistik eignen sich eine Vielzahl verschiedener Programme.
...
Für unsere Zwecke kommen Tabellenkalkulationsprogramme und SPSS aus mehreren Gründen nicht infrage. Demgegenüber sind sowohl Python als auch R prinzipiell gleichermaßen geeignet. Hier ein kleiner Vergleich, der zeigt, wie eine Aufgabe in beiden Umgebungen umgesetzt werden kann.
...
df = pd.read_csv("01-daten/6342800.day", sep=";", comment="#", encoding="latin1")
df["date"] = pd.to_datetime(df["YYYY-MM-DD"], errors="coerce")
df["runoff"] = pd.to_numeric(df[" Calculated"], errors="coerce")
df = df.dropna(subset=["date", "runoff"])
yearly_mean = df.groupby(df["date"].dt.year)["runoff"].mean()

# plt.figure(figsize=(12, 6))
plt.plot(yearly_mean.index, yearly_mean.values, marker="o")
plt.title("Yearly Mean Runoff")
plt.xlabel("Year")
...
ggplot(data = df, mapping = aes(x = Monat)) +
    geom_col(mapping = aes(y = Mittelwert)) +
    geom_errorbar(mapping = aes(ymin = Minimum, ymax = Maximum)) +
    labs(
...
- es einfacher ist, qualitativ hochwertige Grafiken zu erstellen.
